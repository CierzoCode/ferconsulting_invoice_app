...
def update_invoice(invoice_id: str, payload: InvoiceUpdateIn, request: Request, user: dict[str, Any] = Depends(require_roles("admin", "gestor"))):
    ...
    invoice = {
        ...
        **({"invoice_series": clean_invoice_series(payload.invoice_series, payload.invoice_type)} if current_company_slug() == "fincas-lasheras-blanco" else {}),
        ...
        # La fecha es un dato de registro: editar una factura no debe modificarla.
        "invoice_date": current.get("invoice_date") or (payload.invoice_date or date.today()).isoformat(),
        "client_id": payload.client.id,
        "client_name": payload.client.name,
        "client_tax_id": payload.client.tax_id,
        "client_address": payload.client.address,
